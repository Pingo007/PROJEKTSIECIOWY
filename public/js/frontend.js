...
socket.on('updateProjectiles', (backEndProjectiles) => {
  ...
  for (const frontEndProjectileID in frontEndProjectiles) {
    //jezeli nie istnieje w backendzie to usun z players z frontend
...
//odebranie emimtowanego obiektu players z backendu (app.js)
socket.on('updatePlayers', (backEndPlayers) => {

    //Wywalenie graczy ktorych nie 
    ...
    for (const id in backEndPlayers) {
      ...
      //jezeli we frontend nie istnieje gracz z id z backend to jest dodawany nowy player z parametrami x,y backendowego playera
...
function animate() {
  ...
  for (const id in frontEndPlayers) {
    ...
    //if(frontEndPlayers[socket.id]?.room === frontEndPlayers[id]?.room)
    frontEndPlayer.draw()
  }
...
document.querySelector('#usernameForm').addEventListener('submit', (event) => {
  // Zatrzymanie domyślnego działania formularza
  ...
  if (username === "") {
    alert("Wprowadź nazwę użytkownika!"); // Ostrzeżenie, jeśli pole jest puste
    return;
  }
  ...
  if (!selectedRoom) {
    alert("Wybierz pokój przed rozpoczęciem!"); // Ostrzeżenie, jeśli pokój nie został wybrany
    return;
  }
  ...
  socket.emit('initGame', {
    username: username,
    room: selectedRoom, // Przekazanie wybranego pokoju
    width: window.innerWidth, // Szerokość okna
    height: window.innerHeight, // Wysokość okna
    devicePixelRatio: window.devicePixelRatio, // Obsługa ekranów HiDPI
  });
});
